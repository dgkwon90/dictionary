...
## 부록: #17 Settings 시점 API key 저장 재검토 (2026-07-12)
- **재검토 결과: env-only 유지(변경 없음).** #6에서 미결로 남겼던 "OS 키체인·암호화 저장을 Settings UX 시점에 재검토"를 #17에서 수행한 결론이다.
- **근거 — config 계층 분리 원칙(#17에서 확정)**: 설정을 두 계층으로 나눈다. (1) **`.env`(환경변수)** = 부트스트랩/배포 구분·저빈도 시작 옵션(`NEULSANG_DB_PATH`, `NEULSANG_ADDR`, `NEULSANG_AI_PROVIDER`, `NEULSANG_GEMINI_MODEL`, **`NEULSANG_GEMINI_API_KEY`**). (2) **`app_settings`(DB, `GET/PUT /v1/settings`)** = 서비스 동작 정책·운영 튜닝(알림 허용, 아침/저녁 복습 시간 등). `app_settings`는 **평문 정책 저장소**이므로 secret을 넣지 않는다. API key는 "저빈도 시작 옵션(secret)"이라 계층상 `.env`에 속하며, 이는 "평문 저장 금지"를 저장 자체를 안 함으로 충족한 #6 결론과 일관된다.
- **Settings UI 취급**: API key는 값이 아니라 **설정 유무(configured: bool)만 마스킹 표시**, 편집은 `.env`로 안내(읽기전용). provider/model/DB경로/host:port도 동일하게 읽기전용 표시(효력은 프로세스 재시작 시점).
- **미채택 — OS 키체인 / 암호화 DB**: 로컬 단일 사용자 앱에서 키체인은 플랫폼별 연동(keyring)·마스터키 관리 복잡도를 더하고, 암호화 DB는 마스터키를 어디에 둘지 문제가 남아 실익이 낮다. env-only가 지금 위협모델(로컬 파일 접근자에겐 어차피 노출)에 충분. 필요 시 별도 보안 이슈로 재검토(되돌리기 쉬움 → 신규 ADR 불요, 부록으로 기록).
