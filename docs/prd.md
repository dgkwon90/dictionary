...
* macOS, Windows, Linux에서 실행 가능해야 한다. **[v0.1.x 범위 축소, ADR-0009]** 검증 가능한 실기기가 없어 Linux는 현재 릴리스 범위에서 제외하고 후속(미정)으로 미룬다. macOS(arm64/x86_64)·Windows 11만 지금 지원 대상이다. 장기 비전(§0/§1.1)은 변경 없음 — 검증 가능해지면 재개.
* 앱은 시스템 트레이에 상주해야 한다.
