...
### [ ] RW-01 Sidecar 연결 계약과 로컬 API 신뢰 경계 확립 (`L`)
...
### [ ] RW-03 Graceful sidecar 종료와 중단 작업 복구 (`L`)
...
### [ ] RW-04 JSON backup v2와 기능 단위 복원 (`L`)
...
- PRD의 Linux 지원을 유지한다면 CI에 deb/AppImage와 네이티브 smoke를 추가한다. 아니면 PRD 지원 범위를 명시적으로 축소한다.
