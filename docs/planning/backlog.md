...
- **알려진 한계/후속(#13-follow)**: (a) 셸이 SIGKILL/패닉 등 비정상 종료되면 RunEvent::Exit 미발생 → desktopd 고아 프로세스가 48989 포트·SQLite를 붙든 채 남아 다음 기동 실패 가능. 정상 종료(Quit/Cmd+Q/창 닫기)는 자식 정리됨. → desktopd 부모-사망 감시(watchdog) 또는 포트 점유 자동복구로 별도 처리. (b) 창 닫기=앱 종료(현재). 트레이 상주 앱이면 hide-to-tray가 자연스러우나 (a)와 얽혀 있어 의식적 결정 필요.
