...
export default function Dashboard() {
  ...
  if (error) {
    return (
      <div className="db-center">
        <p className="db-error">불러오지 못했어요: {error}</p>
  ...
  if (!summary) {
    return <div className="db-msg">아직 기록이 없어요.</div>;
  }
  ...
  return (
    <div className="db">
      ...
      <section className="db-stats">
        ...
        <Stat label="복습할 카드" value={summary.due_card_count} />
      </section>
        ...
        {summary.category_weakness.length === 0 ? (
          ...
          <ul className="db-weakness">
            {summary.category_weakness.map((c) => (
              <li key={c.category}>
                <span className="db-cat">{categoryLabel(c.category)}</span>
