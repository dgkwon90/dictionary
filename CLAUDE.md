...
백로그 #11 완료(2026-07-08): mastery/weakness score(PRD §13.2~13.3). `internal/domain/review/scoring.go` 순수함수 `MasteryScore`(clamp 0..1)·`WeaknessScore`(0 floor). `Grade` 트랜잭션에서 review_log 삽입 후 `gradeCountsForKnowledgeItem`(item의 모든 카드 review_logs를 `source='review'` 필터로 rating별 집계)으로 전체 이력 재계산 → `learner_items.mastery_score` upsert(응답에 mastery_score 포함). weakness는 컬럼 없어 파생값(정렬/#12 대시보드용, recent_repeat_bonus MVP 0). codex CLEAN 리뷰(source 필터·다중카드 테스트 반영), agy는 여전히 쿼터 소진 skip. **다음 작업: #12 Dashboard summary API(§15.7, `stats` 도메인 신설, weakness 소비) — depends #11(완료).**
