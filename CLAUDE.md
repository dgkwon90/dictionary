...
다음 작업: `docs/planning/backlog.md` #10 (Review session + FSRS-lite 스케줄링) — depends #9(완료). `POST /v1/reviews/session/start`(§15.5)·`POST /v1/reviews/{card_id}/grade`(§15.6), Again/Hard/Good/Easy 간격(§13.1), `review_logs` append-only. **다음 세션에서 착수 예정.**
