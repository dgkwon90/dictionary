...
백로그 #1·#2 완료: `go.work` + `apps/desktopd`(module `neulsang/desktopd`), `GET /healthz`, config/slog. 로컬 API 기본 주소 `127.0.0.1:48989`(`NEULSANG_ADDR`). SQLite 스키마(PRD §11 전체 + `captures.inbox_status`) 마이그레이션은 앱 시작 시 자동 적용 — 드라이버 `modernc.org/sqlite`(CGO 불필요), 방식은 ADR-0007. DB 경로 `NEULSANG_DB_PATH`.
다음 작업: `docs/planning/backlog.md` #3 (Capture API).
