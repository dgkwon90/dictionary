...
브랜치 **`redesign/schema-v2-triage`가 main(2026-07-24) 대비 30커밋 앞서 있고 아직 머지되지 않았다.** 재설계 v2 본체와 그 뒤의 하드닝이 전부 이 브랜치에 있다.

**미사용 의존성 제거와 보안 4항목은 끝났다**(2026-08-18): ① SYNC_URL https 강제, ② 백업 임시파일+rename(재백업 버그 동반 수정), ③ export에도 행 상한 적용 + 200MiB 근거 문서화, ④ 토큰 로그 필드 제거. **④는 애초에 프로덕션에서 안 타는 경로였고**(Tauri가 항상 토큰을 주입), 로그 파일 유출도 없었다 — 위생 차원의 선제 정리다. 백업 화면은 `tauri dev`에서 사람이 확인했다(같은 파일명 재백업 성공, 백업 파일 쿼리 가능, 임시파일 잔여물 없음). 남은 것은 우선순위 순:

1. **백로그 #33** — AI 타임아웃(호출당 20s×3)을 실측 분포 근거로 재조정.
