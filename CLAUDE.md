...
백로그 #1~#3 완료: `apps/desktopd`(module `neulsang/desktopd`) — `GET /healthz`, `POST /v1/captures`(capture+lookup_job+outbox 원자 저장, PRD §15.1), SQLite 자동 마이그레이션(PRD §11 + `captures.inbox_status`, ADR-0007, 드라이버 modernc). 기본 주소 `127.0.0.1:48989`(`NEULSANG_ADDR`), DB 경로 `NEULSANG_DB_PATH`. 계층: domain(interface) ← db/sqlite(구현), transport/http/handlers.
다음 작업: `docs/planning/backlog.md` #4 (AI Explain pipeline, mock provider).
