...
백로그 #1~#4 완료: `apps/desktopd`(module `neulsang/desktopd`) — `GET /healthz`, `POST /v1/captures`(capture+lookup_job+outbox 원자 저장, PRD §15.1), capture 생성 직후 동기적으로 `Explainer`(mock) 실행 → `GET /v1/captures/{id}/explanation`(PRD §15.2, PRD §12.1 ExplainResult 검증·explanations 저장). SQLite 자동 마이그레이션(PRD §11 + `captures.inbox_status`, ADR-0007, 드라이버 modernc). 기본 주소 `127.0.0.1:48989`(`NEULSANG_ADDR`), DB 경로 `NEULSANG_DB_PATH`. 계층: domain(interface: capture/explain) ← db/sqlite(구현), transport/http/handlers.
다음 작업: `docs/planning/backlog.md` #5 (Inbox API).
